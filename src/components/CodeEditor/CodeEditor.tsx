...
export default function CodeEditor() {
  const [code, setCode] = useState<string>(""); // Used in textarea and analysis logic
  const [analysis, setAnalysis] = useState<AnalysisResult>({ // Used in rendering analysis results
    timeComplexity: "Not analyzed yet",
    spaceComplexity: "Not analyzed yet",
    suggestions: "No suggestions yet",
    correctedCode: ""
  });
  const [loading, setLoading] = useState<boolean>(false); // Used in button and spinner logic
  const [language, setLanguage] = useState<string>("java"); // Used in select dropdown and analysis logic
  ...
  const analyzeCode = useCallback(async () => { // Used in button click handler
    if (!code.trim()) {
      setAnalysis(prev => ({
        ...prev,
